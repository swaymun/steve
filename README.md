...
**Development preview.** Source installation, paired iMessage file and video delivery with optional system audio, saved preferences across restart, and a one-time reminder across restart have been exercised on a Mac. Phone takeover and video playback on an actual iPhone, plus recurring tasks, still need live acceptance. The optional Link payment adapter has fixture tests only and is not connected to the operator. This source tree is MIT licensed; no notarized binary release is claimed.
