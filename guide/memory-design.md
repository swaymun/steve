...
Agent name and personality are explicit local settings, separate from user facts and access permissions. Both coordinator and worker receive the current identity. Personalization can change tone; it cannot authorize actions.

Steve's durable store owns stated preferences and dated plan summaries. Workers receive the current active preferences with statement dates and source types. The private, atomically generated `STEVE_MEMORY.md` is read-only to workers and excluded from Git. Corrections replace current values; forgetting removes active values without claiming to erase conversation history. Tentative plans remain distinct from verified commitments.
