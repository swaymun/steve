...
- **Shared desktop:** current source lets computer workers overlap, but they see the same Mac and browser profile. Independent windows and apps work best; simultaneous edits to one page may interfere. The v1.0.0 download still runs computer tasks one at a time. Phone sign-in temporarily pauses workers for private input. Native helpers handle public research only and require compatible Codex support; workers work alone when helpers are unavailable.
