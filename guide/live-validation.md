...
| Shared Mac | Queue two independent visible-browser tasks alongside research. | Both computer workers start within the configured worker limit; each re-observes its own page and returns the correct result. Test overlapping changes to one page separately. |
