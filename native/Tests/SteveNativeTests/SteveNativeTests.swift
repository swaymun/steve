...
final class SteveNativeTests: XCTestCase {
    ...
    func testPromptUsesTheImessageContractAndPlainTextLimits() {
        ...
        XCTAssertTrue(relay.contains("do not perform the task yourself"))
    ...
    func testCodexTurnAccumulatorSeparatesCommentaryFromFinalAnswer() throws {
        var accumulator = CodexTurnAccumulator(threadID: "thread-1", turnID: "turn-1")

        let commentaryDelta: [String: Any] = [
            "method": "item/agentMessage/delta",
            "params": [
                "threadId": "thread-1",
                "turnId": "turn-1",
                "itemId": "item-1",
                "delta": "I updated the app."
            ]
        ]
        XCTAssertNil(accumulator.consume(commentaryDelta))

        let commentaryCompleted: [String: Any] = [
            ...
            "params": [
                ...
                "item": [
                    "id": "item-1",
                    "type": "agentMessage",
                    "text": "I updated the app.",
                    "phase": "commentary"
                ]
            ]
        ]
        XCTAssertEqual(accumulator.consume(commentaryCompleted), .init(phase: .commentary, text: "I updated the app."))

        let finalCompleted: [String: Any] = [
            "method": "item/completed",
            "params": [
                "threadId": "thread-1",
                "turnId": "turn-1",
                "item": [
                    "id": "item-2",
                    "type": "agentMessage",
                    "text": "The change is installed.",
                    "phase": "final_answer"
                ]
            ]
        ]
        XCTAssertEqual(accumulator.consume(finalCompleted), .init(phase: .finalAnswer, text: "The change is installed."))
        XCTAssertEqual(accumulator.result().text, "The change is installed.")
    }

    func testCodexTurnAccumulatorTreatsMissingPhaseAsFinalOnly() {
        var accumulator = CodexTurnAccumulator(threadID: "thread-1", turnID: "turn-1")
        ...
        XCTAssertEqual(accumulator.consume(completed), .init(phase: nil, text: "Compatibility response."))
        XCTAssertEqual(accumulator.result().text, "Compatibility response.")
    }
