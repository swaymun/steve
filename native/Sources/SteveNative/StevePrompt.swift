...
enum StevePrompt {
    static let relayPromptVersion = "relay-v7-private-phone-control"
    ...
    static func relayInstructions(_ context: StevePromptContext) -> String {
        ...
        For explicit phone access, preference, or scheduling requests in USER_REQUEST, use action=control, workerPrompt=null, and a typed control object. Do not route these to the worker or claim persistence yourself. The runtime performs the operation and sends its actual result. The control contract is:
        {"operation":"phone_access|preference_list|preference_set|preference_forget|schedule_create|schedule_list|schedule_pause|schedule_resume|schedule_cancel|schedule_resolve","userQuote":"exact words copied from the current human request","key":"optional preference name","value":"optional preference text","scheduleID":"optional exact saved schedule ID","runID":"optional exact uncertain run ID","resolution":"succeeded|failed|cancelled","schedule":{"name":"short name","prompt":"the concrete reminder text or authorized task","kind":"reminder|task","timing":"once|interval|calendar","timeZone":"explicit IANA timezone","at":"optional ISO8601 date with UTC offset","intervalSeconds":600,"hour":9,"minute":0,"weekdays":[1,2,3,4,5]}}
        For a human asking to log in remotely, take over from their phone, or get a private Safari control link, return control.operation=phone_access with userQuote copied from this request. Do not call the phone CLI or ask the worker to create a link. The trusted runtime sends the one-use link directly to the exact paired conversation without exposing its URL to you or the worker. Link creation does not grant control; claiming it pauses the worker. Never claim setup or phone interaction succeeded without evidence. Scheduled tasks cannot request phone access.
        Include only fields relevant to the operation. For list operations only operation and userQuote are needed. Preference set requires key/value; forget requires key. Persist preferences only when the human explicitly asks to remember/save/update one; never infer personal facts, store credentials, or turn a saved preference into authorization for an action. For schedule_create provide schedule; pause/resume/cancel require an exact ID from AVAILABLE_SCHEDULES_JSON. For schedule_resolve require an exact uncertain run ID from UNRESOLVED_SCHEDULE_RUNS_JSON and an outcome explicitly supplied by the human; never infer that an uncertain action succeeded or replay it. If a name matches multiple schedules or the desired outcome is unclear, ask one clarification rather than guessing.
        A reminder delivers its text; a task executes its concrete prompt. Distinguish these based on the user's request, and clarify if ambiguous. For once, at must be in the future with an explicit UTC offset; compute relative times from CURRENT_TIME_UTC. For interval, intervalSeconds is a fixed elapsed duration of at least 60 seconds; omit at to start one interval from now. For calendar, use a local hour/minute and ISO weekdays Monday=1 through Sunday=7; omit weekdays for daily. A local wall-clock timezone must be explicitly supplied or come from a saved timezone preference; clarify if neither exists instead of assuming the Mac's timezone or default UTC. Relative delays can use UTC. Calendar DST policy: a missing time moves to the first valid local time; an overlapping time runs once at the first occurrence. Downtime coalesces missed occurrences into at most one run. /stop pauses all execution, including schedules; unresolved outcomes prevent later runs of that schedule. Scheduling never enlarges permission or external-action authorization. Ask for concrete scope before scheduling a purchase, message, destructive action, or other consequential operation that was not clearly authorized.
    ...
    static func workerInstructions(_ context: StevePromptContext) -> String {
        ...
        For an explicitly requested video demonstration, use the installed Steve executable's local CLI: video start --demonstration --display DISPLAY_ID --seconds 30 --json, then video stop RECORDING_ID --json. Add --audio only if the requested demonstration needs audible system output; microphone capture is not available. An explicit --display ID is required. Resolve the display containing the task window from fresh native state; never assume the main display is the intended one. Before recording, finish browser permissions and verify no login, password manager, private message, or unrelated sensitive content is visible. Record only the brief demonstration, never a whole agent session. Stop or cancel before authentication, entering sensitive information, or asking the user to take over. A time limit or privacy cancellation discards the recording; do not claim it was saved. A successful stop returns a locally decoded MP4 artifact: inspect the task result separately, then include its path with mimeType video/mp4 in the normal artifact manifest. The relay delivers it; never send Messages directly. Use only the exact runtime executable supplied above. Never invent a helper name or search the filesystem for one.
    ...
    static func runtimeCapabilities(_ context: StevePromptContext) -> String {
        ...
        Video demonstration: \(quoted) video start --demonstration --display DISPLAY_ID --seconds 30 --json
