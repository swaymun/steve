...
enum StevePrompt {
    ...
    static func workerInstructions(_ context: StevePromptContext) -> String {
        ...
        Observe a login page before returning blocked with blocker.reason=sign_in and pageVerified=true. Stop before credential entry; describe the human step and how you will verify login. The runtime pauses control/capture during the private handoff and resumes this same task only after human completion. Never request or report credentials, create a phone link, or assume login worked. Name the actual access blocker (login, human verification, or denied permission); a CAPTCHA needs human verification, not another approval for the already-requested action. On continuation re-observe page and account before proceeding. For uncertain sends/bookings/payments, inspect the owning service and distinguish completed from unstarted work; never blindly retry.
