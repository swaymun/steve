    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if model.codexUnavailable { Text("Steve is paused because Codex is unavailable.").font(.caption).foregroundStyle(.red).padding(.vertical, 8) }
                ...
                disclosure("Permissions", value: model.snapshot?.settings.permissionProfile.map(humanizeLabel) ?? "Select a profile", expanded: $permissionsExpanded) { ForEach(model.snapshot?.permissions ?? []) { profile in action(humanizeLabel(profile.name ?? profile.id), subtitle: profile.description ?? permissionDescription(profile.id), disabled: !profile.allowed) { model.selectPermission(profile.id) } } }
...
private func permissionDescription(_ value: String) -> String { switch value.lowercased() { case "read-only": return "Read files without making changes."; case "workspace-write": return "Read and edit files inside the workspace."; case "danger-full-access": return "Full computer access, including outside the workspace."; default: return "Controls what Steve can do with your workspace." } }
